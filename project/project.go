// Package project proporciona utilidades para proyectar, formatear y adaptar fechas UTC
// almacenadas en la base de datos a la zona horaria particular de cada usuario o cliente.
//
// Autor: Jhonatan.
package project
...
func ForUser(utcTime time.Time, userZone string, customLayout ...string) (UserTime, error) {
	loc, err := timezoner.LoadLocation(userZone)
	if err != nil {
		return UserTime{}, fmt.Errorf("project: no se pudo resolver la zona del usuario '%s': %w", userZone, err)
	}
	...
	isDST, _ := timezoner.IsDST(loc.String(), utcTime)
	...
	return UserTime{
		...
		OffsetFormatted: timezoner.FormatOffset(offset),
		IsDST:           isDST,
...
func BatchForUsers(utcTime time.Time, userZones []string) (map[string]UserTime, error) {
	if len(userZones) == 0 {
		return nil, timezoner.ErrNoZonesProvided
	}
