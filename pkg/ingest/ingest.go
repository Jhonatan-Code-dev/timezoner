...
func FromLocal(localTime time.Time, sourceZone string) (time.Time, error) {
	loc, err := zone.LoadLocation(sourceZone)
	if err != nil {
		return time.Time{}, fmt.Errorf("%w: zona de origen inválida '%s'", zone.ErrInvalidZone, sourceZone)
	}
...
func FromString(dateStr, defaultZone string) (time.Time, error) {
	...
	z := strings.TrimSpace(defaultZone)
	if z == "" {
		z = "UTC"
	}
